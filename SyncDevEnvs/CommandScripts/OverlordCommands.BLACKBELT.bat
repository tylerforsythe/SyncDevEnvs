
del /Q "{SYNC_DIR_PATH}\STATUS_{DATE}.txt"

"C:\Program Files\Microsoft SQL Server\110\Tools\Binn\SQLCMD.EXE" -S BLACKBELT -d IMPrintingLucy -Q "BACKUP DATABASE IMPrintingLucy TO DISK = '{TEMP_DIR_PATH}\IMPrinting_{DATETIME}.BAK'"

"C:\Program Files\7-Zip\7z" a -mmt -t7z -mx1 "{SYNC_DIR_PATH}\IMPrinting_{DATETIME}.7z" {TEMP_DIR_PATH}\IMPrinting_{DATETIME}.BAK -

@echo COMPLETE {DATETIME}> "{SYNC_DIR_PATH}\STATUS_{DATE}.txt"

del /Q "{TEMP_DIR_PATH}\IMPrinting_{DATETIME}.BAK"
